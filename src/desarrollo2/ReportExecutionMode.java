...
public class ReportExecutionMode implements ExecutionModeRunnerInterface {
	...
	public int execute(final Options options, final ConfigurationFile configFile) throws IOException {
		...
		try {
			outputWriter = new BufferedWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream(options.getIndicatorsFile()), "UTF-8")));
			...
			for (IntegrityCheck session : sessions) {
				...
				String report = this.getReport(session, previousRatio, currentRatio);
	...
	private String getReport(final IntegrityCheck session, final float previousRatio, final float currentRatio) {
		String report = "\n";
