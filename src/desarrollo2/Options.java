...
class Options {
	...
	private static Options getDefaults() {
		...
		return new Options(ExecutionMode.HASH_AND_DAEMON, basePath + "main.conf", basePath + "hashes.lst", basePath + "incidents.txt", basePath + "indicators.txt", HashAlgorithm.SHA1);
	}

	private Options(final ExecutionMode mode, final String configFile, final String hashesFile, final String issuesFile, final String indicatorsFile, final HashAlgorithm algorithm) {
	 ...
	 */
	static Options fromArguments(final String[] args) {
	 ...
	 * 
	 * @throws IOException
	 *             Archivo no encontrado, no leible o escribible.
	 */
	void checkConfigFile() throws IOException {
		this.checkFilePath(this.configFile, false);
	}
	 ...
	 * 
	 * @throws IOException
	 *             Uno de los archivos no ha sido encontrado, no es legible o escribible.
	 */
	void checkFiles() throws IOException {
		...
		case REPORT:
			this.checkFilePath(this.configFile, false);
			this.checkFilePath(this.incidentsFile, false);
			this.checkFilePath(this.indicatorsFile, true);
			break;
		}
	}
	 ...
	 *            Ruta al archivo.
	 * @param isOutput
	 *            Si es true se considerará al archivo 'de salida', 'de entrada' en otro caso.
	 * @throws IOException
	 *             Si se produce algún error en las comprobaciones.
	 */
	private void checkFilePath(final String path, final boolean isOutput) throws IOException {
	...
	void setHashesFile(final String hashesFile) {
		this.hashesFile = hashesFile;
	}
	...
	void setIncidentsFile(final String incidentsFile) {
		this.incidentsFile = incidentsFile;
	}

	String getIndicatorsFile() {
		return this.indicatorsFile;
	}

	void setIndicatorsFile(final String indicatorsFile) {
		this.indicatorsFile = indicatorsFile;
	}
	...
	public int getMinutesBetweenIntegrityChecks() {
		return this.minutesBetweenIntegrityChecks;
	}

	public void setMinutesBetweenIntegrityChecks(final int minutesBetweenIntegrityChecks) {
	...
	public String algorithmToPlatformName() {
		...
		throw new RuntimeException("Internal error: unknown algorithm for Options.algorithmToPlatformName: " + this.getAlgorithm().toString());
	}

	@Override
	public String toString() {
		return "Options {\n" + " Execution mode: " + this.executionMode.toString() + "\n" + " Hash algorithm: " + this.algorithm + "\n" + " Configuration file: " + this.configFile + "\n" + " Hashes file: " + this.hashesFile + "\n" + " Issues file: "
			+ this.incidentsFile + "\n" + " Indicators file: " + this.indicatorsFile + "\n" + " Minutes between integrity checks: " + this.minutesBetweenIntegrityChecks + "\n}";
	}
